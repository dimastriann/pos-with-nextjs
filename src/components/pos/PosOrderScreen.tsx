    ...
    <div className="flex flex-col overflow-hidden h-full">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 overflow-y-auto flex-1">
        {filteredProducts.map((product) => (
          <motion.div key={product.id} whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }} transition={{ duration: 0.12 }}>
            <Card
            ...
            >
              <CardContent className="p-3">
                <p className="font-semibold text-sm leading-tight text-card-foreground">{product.name}</p>
                <p className="text-primary font-bold text-sm mt-1">Rp {product.price.toLocaleString()}</p>
                {product.stock <= 5 && product.stock > 0 && (
                  <Badge variant="secondary" className="text-orange-600 bg-orange-50 text-xs mt-1">
                    Low: {product.stock}
                  </Badge>
                )}
                {product.stock === 0 && (
  ...
  return (
    <>
      ...
      <div className="hidden md:grid grid-cols-12 gap-3 p-3 flex-1 overflow-hidden">
        {/* Product grid */}
        <div className="col-span-7 flex flex-col overflow-hidden">{productPanel}</div>

        {/* Control buttons */}
        <div className="col-span-1 flex flex-col gap-1.5 pt-8">
          <Button size="sm" variant="outline" className="text-xs" onClick={() => dispatch({ type: 'CLEAR_ORDER' })}>
            Clear
          </Button>
          <Button size="sm" variant="outline" className="text-xs" onClick={() => alert('Note coming soon')}>
            Note
          </Button>
          <Button size="sm" variant="outline" className="text-xs" onClick={() => alert('Discount coming soon')}>
            Disc %
          </Button>
        </div>
