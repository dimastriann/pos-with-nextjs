...
export default function UomPage() {
  ...
  const columns = [
    ...
    { header: 'Symbol', accessor: 'symbol' as keyof Uom, className: 'text-muted-foreground' },
  ];
    ...
    <div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={formData.id ? 'Edit Unit' : 'New Unit'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Label>Name</Label>
            <Input required value={formData.name || ''} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Kilogram" />
          </div>
          <div>
            <Label>Symbol</Label>
            <Input required value={formData.symbol || ''} onChange={(e) => setFormData({ ...formData, symbol: e.target.value })} placeholder="e.g. kg" />
          </div>
          <div className="flex gap-3 justify-end pt-4">
            <Button type="button" variant="outline" onClick={() => setIsModalOpen(false)}>Cancel</Button>
            <Button type="submit" disabled={isLoading}>{isLoading ? 'Saving...' : 'Save Unit'}</Button>
          </div>
        </form>
      </Modal>
    </div>
