...
export default function CategoriesPage() {
  ...
  const columns = [
    { header: 'Name', accessor: 'name' as keyof Category, className: 'font-medium' },
    { header: 'Description', accessor: 'description' as keyof Category, className: 'text-muted-foreground' },
  ];
    ...
    <div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={formData.id ? 'Edit Category' : 'New Category'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Label>Name</Label>
            <Input required value={formData.name || ''} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Beverages" />
          </div>
          <div>
            <Label>Description</Label>
            <Textarea className="resize-none" value={formData.description || ''} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Optional description..." />
          </div>
