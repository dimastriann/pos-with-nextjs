...
export default function PaymentMethodsPage() {
  ...
  const columns = [
    { header: 'Name', accessor: 'name' as keyof PaymentMethod, className: 'font-medium' },
    { header: 'Type', accessor: 'type' as keyof PaymentMethod, className: 'text-muted-foreground' },
  ];
    ...
    <div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={formData.id ? 'Edit Method' : 'New Payment Method'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <Label>Type</Label>
            <select className="w-full h-9 rounded-md border border-input bg-background px-3 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-ring" value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value as PaymentMethod['type'] })}>
          ...
          <div className="flex gap-3 justify-end pt-4">
            ...
            <Button type="submit" disabled={isLoading}>{isLoading ? 'Saving...' : 'Save Method'}</Button>
          </div>
        </form>
      </Modal>
    </div>
