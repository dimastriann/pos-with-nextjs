...
export default function OrdersPage() {
  ...
  const columns = [
    ...
    {
      header: 'Total',
      accessor: (o: PosOrder) => `Rp ${o.totalAmount.toLocaleString()}`,
    },
    {
      header: 'Status',
      accessor: (o: PosOrder) => (
        <span
          className={`px-2 py-1 rounded-full text-xs font-bold ${o.status === 'Paid' ? 'bg-green-100 text-green-700' : o.status === 'Draft' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}
        >
          {o.status}
        </span>
      ),
