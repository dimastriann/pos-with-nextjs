...
export default function ShopsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this shop?')) { await shopRepository.delete(id); loadData(); }
  };

  const handleEdit = (item: PosShop) => { setFormData(item); setIsModalOpen(true); };
  const handleAddNew = () => { setFormData({ active: true }); setIsModalOpen(true); };
  ...
  return (
    <div>
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Shops</h1>
          <p className="text-sm text-gray-500 mt-1">Manage physical shop locations.</p>
        </div>
        <div className="flex gap-3">
          <div className="bg-gray-100 p-1 rounded-lg flex text-sm font-medium">
            <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 rounded-md transition-all ${viewMode === 'list' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>List</button>
            <button onClick={() => setViewMode('kanban')} className={`px-3 py-1.5 rounded-md transition-all ${viewMode === 'kanban' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>Kanban</button>
          </div>
          <button onClick={handleAddNew} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium shadow-sm transition-colors text-sm flex items-center gap-2">
            <span>＋</span> Add Shop
          </button>
        </div>
      </div>
      ...
      {viewMode === 'list' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.map((item) => <KanbanCard key={item.id} item={item} onEdit={handleEdit} onDelete={handleDelete} />)}
          {data.length === 0 && <div className="col-span-full text-center py-12 text-gray-500 border-2 border-dashed border-gray-200 rounded-xl">No shops found. Create one to get started.</div>}
        </div>
      )}
